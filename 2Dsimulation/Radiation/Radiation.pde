...
void drawLights() {
  ...
  // 圓的半徑（橢圓）
  float radiusX = (stageRight - stageLeft) * 0.45;
  float radiusY = (stageBottom - stageTop) * 0.40;

  float rayLength = lightHeight;          // 放射線總長度
  float step = rayLength / numSegments;   // 每顆 pixel 的距離
  float dotDiameter = min(lightWidth, step) * 0.9;   // 可以改 0.7/0.8 控制縫隙
  ...
  for (int i = 0; i < numLights; i++) {
    ...
    // 圓心 → 外圈 的方向向量
    float dirX = baseX - centerX;
    float dirY = baseY - centerY;
    float len  = sqrt(dirX*dirX + dirY*dirY);
    dirX /= len;
    dirY /= len;

    // 從外圈往「圓外」方向堆 pixel（如果要往圓心，就改成 baseX - dirX * dist）
    for (int s = 0; s < numSegments; s++) {
