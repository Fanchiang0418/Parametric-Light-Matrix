...
void drawLights() {
  ...
  float radiusX = (stageRight - stageLeft) * 0.55;
  float radiusY = (stageBottom - stageTop) * 0.35;

  float dotDiameter = min(lightWidth, segmentHeight) * 0.8;

  float curveBase = lightHeight * 0.5; // 基本弧度
  ...
  for (int i = 0; i < numLights; i++) {
    ...
    // 原本的直線端點（維持不變）
    float x0 = baseX;
    float y0 = baseY - segmentHeight/2;
    float x1 = baseX;
    float y1 = baseY - segmentHeight/2 - (numSegments-1)*segmentHeight;

    // 外圈多一點弧度（依離中心的水平距離拉弧）
    float distX = abs(baseX - centerX);
    float curve = curveBase * map(distX, 0, radiusX, 0.2, 1.0);

    // ===== 這裡改成「朝中心」 =====
    float dir = (centerX - baseX >= 0) ? 1 : -1; // 中心在右邊 -> 往右；在左邊 -> 往左
    float cx = baseX - dir * curve;              // 控制點往中心側推
    float cy = (y0 + y1) / 2.0;                  // 高度放在中間
    // =================================
