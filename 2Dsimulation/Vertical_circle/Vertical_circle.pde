...
int numSegments = 20;      // 每條燈切成幾個 pixel
...
void drawLights() {
  ...
  float stageTop    = 80;
  float stageBottom = height - 48;

  float centerX = (stageLeft + stageRight) / 2.0;
  float centerY = (stageTop + stageBottom) / 2.0;

  float radiusX = (stageRight - stageLeft) * 0.53;   // 左右寬
  float radiusY = (stageBottom - stageTop) * 0.35;   // 上下（深度）

  float dotDiameter = min(lightWidth, lightHeight / numSegments) * 0.8;

  float circleRadius = lightHeight * 0.5;  // 圓半徑 = 原本燈長度的一半

  for (int i = 0; i < numLights; i++) {

    // 每支燈的「圓心位置」（仍按照你原本的圓形舞台分布）
    float angle = map(i, 0, numLights, 0, TWO_PI);
    float cx = centerX + cos(angle) * radiusX;
    float cy = centerY + sin(angle) * radiusY;
    ...
    for (int s = 0; s < numSegments; s++) {
      ...
      float a = t * TWO_PI;    // 0 → 360°

      float px = cx + cos(a) * circleRadius;
      float py = cy + sin(a) * circleRadius;
