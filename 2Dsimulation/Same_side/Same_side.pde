...
void drawLights() {
  ...
  float stageTop    = 80;
  float stageBottom = height + 24;

  float segmentHeight = lightHeight / numSegments;

  float centerX = (stageLeft + stageRight) / 2.0;
  float centerY = (stageTop + stageBottom) / 2.0;

  float radiusX = (stageRight - stageLeft) * 0.55;   // 左右寬
  float radiusY = (stageBottom - stageTop) * 0.35;   // 上下（深度）

  // 圓點直徑
  float dotDiameter = min(lightWidth, segmentHeight) * 0.8;

  float curveBase = lightHeight * 0.5;   // 整體彎曲的基準，可自行調整

  for (int i = 0; i < numLights; i++) {
    float angle = map(i, 0, numLights, 0, TWO_PI);

    float baseX = centerX + cos(angle) * radiusX;
    float baseY = centerY + sin(angle) * radiusY;

    // 這一支燈原本「直線」的起點 / 終點（保持不變，確保大小一樣）
    float x0 = baseX;
    float y0 = baseY - segmentHeight/2;                            // 下端
    float x1 = baseX;
    float y1 = baseY - segmentHeight/2 - (numSegments-1)*segmentHeight; // 上端

    // 讓外圈多一點弧度（可以拿掉 map，變成固定弧度）
    float curve = curveBase * map(i, 0, numLights-1, 0.3, 1.0);

    // 控制點：決定彎向哪邊、彎多少
    // 想要往左開就用 baseX - curve，往右開就改成 baseX + curve
    float cx = baseX - curve;
    float cy = (y0 + y1) / 2.0;

    for (int s = 0; s < numSegments; s++) {
      float bri = brightness[i][s];
      if (bri <= 0) continue;
      fill(0, 0, bri);  // 白色（HSB：S=0, B=bri）
